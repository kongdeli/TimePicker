...
public class WheelTime {
    ...
    private void setLunar(int year, final int month, int day, boolean isLeap, int h, int m, int s) {
        ...
        if (minuteInterval == 5) {
            ...
            numericWheelAdapter.setMinuteInterval(minuteInterval);
            wv_minutes.setAdapter(numericWheelAdapter);
        } else {
    ...
    private void setSolar(int year, final int month, int day, int h, int m, int s) {
        ...
        if (minuteInterval == 5) {
            NumericWheelAdapter numericWheelAdapter = new NumericWheelAdapter(0, 55);
            numericWheelAdapter.setMinuteInterval(5);
            wv_minutes.setAdapter(numericWheelAdapter);
            m = m / 5;
        } else {
            wv_minutes.setAdapter(new NumericWheelAdapter(0, 59));
...
//                maxItem = endD;
    ...
    public String getTime() {
        ...
        int currentSecond;
        if (minuteInterval == 5) {
            currentMinute = wv_minutes.getCurrentItem() * 5;
            currentSecond = 0;
        } else {
            currentMinute = wv_minutes.getCurrentItem();
            currentSecond = wv_seconds.getCurrentItem();
        }
        if (isLunarCalendar) {
            //如果是农历 返回对应的公历时间
            return getLunarTime();
           ...
           /* int i = wv_month.getCurrentItem() + startMonth;
            ...
            if ((wv_month.getCurrentItem() + startMonth) == startMonth) {
                sb.append((wv_year.getCurrentItem() + startYear)).append("-")
                        ...
                        .append(currentMinute).append(":")
                        .append(currentSecond);
            } else {
                sb.append((wv_year.getCurrentItem() + startYear)).append("-")
                        .append((wv_month.getCurrentItem() + startMonth)).append("-")
